...
///
///   Tools > Echolocation > Remove Old Button Echolocation
///       Strips the superseded button-based Echolocation/EcholocationFollow components
///       from the open scene. Also run automatically by "Add System To Open Scene".
/// </summary>
public static class EchoSceneAugmentor
{
    ...
    public static void AddSystemToOpenScene()
    {
        ...
        // Now that the new system is wired in, strip the old button-based echolocation.
        int oldRemoved = RemoveOldEchoComponents();

        MarkDirty();
        EditorUtility.DisplayDialog("Echolocation",
            "System added to the open scene.\n\n" +
            "Player now has: small automatic footsteps (every 1s) + a large wave on the E key.\n" +
            (oldRemoved > 0 ? "Removed " + oldRemoved + " old button-echolocation component(s).\n" : "") +
    ...
    [MenuItem("Tools/Echolocation/Remove Old Button Echolocation")]
    public static void RemoveOldEcholocation()
    {
        int n = RemoveOldEchoComponents();
        if (n > 0) MarkDirty();
        EditorUtility.DisplayDialog("Echolocation",
            n > 0
                ? "Removed " + n + " old echolocation component(s) (Echolocation / EcholocationFollow) from the open scene.\n\n" +
                  "If 'Echolocation.cs' is no longer used by any scene, you can now delete the script too."
                : "No old button-echolocation components found in the open scene.",
            "OK");
    }

    /// <summary>
    /// Removes the superseded button-based echolocation components from the open scene.
    /// Matched by type name via reflection so this tool doesn't hard-depend on those
    /// scripts (they can be deleted afterwards without breaking this file).
    /// </summary>
    private static int RemoveOldEchoComponents()
    {
        int removed = 0;
        foreach (var mb in Object.FindObjectsOfType<MonoBehaviour>(true))
        {
            ...
            if (typeName == "Echolocation" || typeName == "EcholocationFollow")
            {
                Undo.DestroyObjectImmediate(mb);
                removed++;
            }
        }
        if (removed > 0) Debug.Log("[Echo] Removed " + removed + " old button-echolocation component(s).");
        return removed;
    }
